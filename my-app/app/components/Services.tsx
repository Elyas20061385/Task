    ...
    <section className="py-24 bg-[#F8F9FA] relative overflow-hidden">
      ...
      <div className="max-w-7xl mx-auto px-6 relative z-10">
        ...
        <div className="flex flex-col md:flex-row justify-between items-end mb-16 gap-6">
          <div className="max-w-2xl">
            <motion.span 
              ...
              className="text-[#FF7A5C] font-bold tracking-widest uppercase text-sm mb-4 block"
            >
              Our Expertise
            </motion.span>
            <motion.h2 
            ...
            >
              ...
              <span className="text-transparent bg-clip-text bg-linear-to-r from-gray-900 to-gray-500">Modern Living.</span>
            </motion.h2>
          </div>
          <motion.p 
            initial={{ opacity: 0 }}
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
          {services.map((service, index) => (
            ...
            >
              ...
              <div className="absolute top-0 right-0 w-24 h-24 bg-gray-50 rounded-bl-[100px] -z-10 group-hover:bg-[#FF7A5C]/5 transition-colors" />
              ...
              <h3 className="text-xl font-bold text-gray-900 mb-4 group-hover:text-[#FF7A5C] transition-colors">
                {service.title}
              </h3>
              ...
              <button className="flex items-center gap-2 text-sm font-bold text-gray-900 group/btn">
                Learn More 
                <div className="w-8 h-8 rounded-full bg-gray-100 flex items-center justify-center group-hover/btn:bg-[#FF7A5C] group-hover/btn:text-white transition-all">
        ...
        <motion.div 
          initial={{ opacity: 0 }}
          whileInView={{ opacity: 1 }}
          className="mt-20 p-8 rounded-[40px] bg-gray-900 flex flex-col md:flex-row items-center justify-between gap-8"
        >
          <div className="flex items-center gap-6">
            <div className="flex -space-x-4">
              {[1,2,3,4].map(i => (
                <div key={i} className="w-12 h-12 rounded-full border-4 border-gray-900 bg-gray-700 overflow-hidden">
                   <img src={`https://i.pravatar.cc{i+20}`} alt="team" />
                </div>
              ))}
            </div>
            <p className="text-white font-medium text-lg">آماده‌اید با تیم متخصص ما صحبت کنید؟</p>
          </div>
          <button className="bg-white text-gray-900 px-8 py-4 rounded-2xl font-bold hover:bg-[#FF7A5C] hover:text-white transition-all active:scale-95 whitespace-nowrap">
            Book a Consultation
          </button>
        </motion.div>
